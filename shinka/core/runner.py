...
class EvolutionRunner:
    def __init__(
        ...
        self.db = ProgramDatabase(
            config=db_config, embedding_model=evo_config.embedding_model
        )
    ...
    def _save_experiment_config(
        ...
        """Save experiment configuration to a YAML file and .hydra directory."""
        config_data = {
        ...
        # Save .hydra directory for child eval processes to find config
        if self.hydra_cfg is not None:
            hydra_dir = Path(self.results_dir) / ".hydra"
            hydra_dir.mkdir(parents=True, exist_ok=True)
            
            # Save the full config
            OmegaConf.save(self.hydra_cfg, hydra_dir / "config.yaml")
            
            # Save hydra node if present
            hydra_node = self.hydra_cfg.get("hydra", {})
            if hydra_node:
